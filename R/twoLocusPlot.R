...
#'
#' The `coeff` parameter must be one of the following:
#'
#' "kinship", "phi", "phi11", "k00", "k01", "k02", "k10", "k11", "k12", "k20",
#' "k21" or "k22".
#'
...
twoLocusPlot = function(peds, coeff = "k11", xlab = "Recombination rate",
  ...
  # Compute coefficient value for each rho
  if(coeff %in% c("kinship", "phi", "phi11")) {
    ...
    kvals = sapply(peds, function(x)
      sapply(rseq, function(r) twoLocusIBD(x$ped, x$ids, r, coefs = coeff)))
